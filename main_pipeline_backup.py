...
3. Upload good training files to GCP databasepython3
"""
Main Pipeline for    console_handler = logging.StreamHandler()
    ...
    return loggermiconductor Fault Detection
=========================================================
...
def setup_main_logger():
    ...
    file_handler = logging.FileHandler('logs/main_pipeline.log')
    file_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    file_handler.setFormatter(file_formatter)
    logger.addHandler(file_handler)
...
class MainPipeline:
    ...
    def step_1_create_training_schema(self):
        ...
        try:
            from schema.create_training_json_schema import create_training_schema
            schema_path = create_training_schema()
            self.logger.info(f"✅ Training schema created: {schema_path}")
            return True
    ...
    def step_2_validate_training_data(self):
        ...
        try:
            from src.data_validation.training_data_validation import validate_training_data
            validation_result = validate_training_data(self.training_data_dir)
    ...
    def step_6_validate_prediction_data(self):
        ...
        try:
            from src.data_validation.prediction_data_validation import validate_prediction_data
            validation_result = validate_prediction_data(self.prediction_data_dir)
            if validation_result:
